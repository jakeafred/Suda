...
public class SudaPlayer extends ListenerAdapter {
	...
	private final Map<String, SudaManager> musicManagers;
	...
	public void onMessageReceived(MessageReceivedEvent event)
	{
		...
		SudaManager mng = getMusicManager(guild);
		...
		if (".join".equals(command[0]))
		{
			...
			else
			{
				...
				if (chan == null)
				{
					event.getChannel().sendMessage("I couldn't find the VoiceChannel by the name: " + command[1]).queue();
				}
				else
				{
					...
					catch (PermissionException e)
					{
						...
						{
							event.getChannel().sendMessage("I do not have permission to connect to " + chan.getName()).queue();
						}
					}
				}
			}
			...
			else    //Commands has 2 parts, .play and url.
			{
				loadAndPlay(mng, event.getChannel(), command[1], false);
                 ...
                 catch (PermissionException e)
                 {
                     if (e.getPermission() == Permission.VOICE_CONNECT)
                     {
                         event.getChannel().sendMessage("Yui does not have permission to connect to: " + event.getMember().getVoiceState().getChannel().getName()).queue();
                     }
}
			}
		}
	...
	private void loadAndPlay(SudaManager mng, final MessageChannel channel, String url, final boolean addPlaylist)
	{
	...
	private SudaManager getMusicManager(Guild guild)
	{
		String guildId = guild.getId();
		SudaManager mng = musicManagers.get(guildId);
		if (mng == null)
		{
			synchronized (musicManagers)
			{
				...
				if (mng == null)
				{
					mng = new SudaManager(playerManager);
					mng.player.setVolume(30);
